...
Скачать PDF описи вложения, сформированной сервисом, по отправлению конкретному получателю. Опись формируется после передачи в Почту, но не мгновенно: пока документ не готов — `404`, следует повторить запрос позже (готовность видна по полю `inventory_pdf` в письме). Требуется scope `letters:read`.
